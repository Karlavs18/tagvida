...
export default function ScanPage() {
  ...
  function initMap(L, lat, lng) {
    ...
    L.marker([lat, lng], { icon }).addTo(map)
      .bindPopup('<b>' + dije.name + '</b><br>Escaneado aqui').openPopup()
  }
  ...
  async function triggerNotification(location) {
    ...
    const mapsUrl = location && location.lat
    ...
    const lineas = [
      ...
      mapsUrl,
      '',
      'Direccion: ' + (location && location.address ? location.address : 'no disponible'),
      'Hora: ' + new Date().toLocaleString('es-MX', { timeZone: 'America/Mexico_City' }),
    ]
    if (finderPhone) lineas.push('Mi numero: ' + finderPhone)
    lineas.push('', 'Como puedo devolverte tu mascota?')

    const phone = (dije.owner_whatsapp || '').replace(/\D/g, '')
    const waUrl = 'https://wa.me/' + phone + '?text=' + encodeURIComponent(lineas.join('\n'))

    setTimeout(function() { window.open(waUrl, '_blank') }, 3000)
    setTimeout(function() { setNotified(true) }, 2000)
  }
  ...
  async function sendMessage() {
    ...
    const lineas = [
      ...
      mapsUrl,
      '',
      'Direccion: ' + (userLoc ? locLabel.replace('📍 ', '') : 'no disponible'),
      'Hora: ' + new Date().toLocaleString('es-MX', { timeZone: 'America/Mexico_City' }),
    ]
    if (finderPhone) lineas.push('Mi numero: ' + finderPhone)
    lineas.push('', 'Mensaje: "' + msgValue + '"')
  ...
  function abrirWhatsapp() {
    ...
    const lineas = [
      '*Tag Vida - Encontre a ' + dije.name + '!*',
      '',
      userLoc ? 'https://maps.google.com/?q=' + userLoc.lat + ',' + userLoc.lng : 'sin ubicacion',
      '',
      'Direccion: ' + locLabel.replace('📍 ', ''),
  ...
  return (
    <>
      <Head>
        <title>Tag Vida — Encontre a {dije.name}!</title>
      ...
      <div style={{ background:'#F9F7F4', minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', padding:'1.5rem 1rem 3rem', position:'relative' }}>
        ...
        <div style={{ width:'100%', maxWidth:'420px', background:'white', borderRadius:'28px', border:'2px solid #F3F4F6', boxShadow:'0 20px 60px rgba(0,0,0,.08)', overflow:'hidden', position:'relative', zIndex:1, marginBottom:'1rem' }}>
          ...
          <div style={{ background:'linear-gradient(135deg,#FFF3EE,#FFF8F5)', borderBottom:'1.5px solid #FFD4C2', padding:'1rem 1.5rem', display:'flex', alignItems:'center', gap:'.8rem' }}>
            <span style={{ width:'12px', height:'12px', borderRadius:'50%', background:'#FF6B35', display:'block', flexShrink:0 }} />
            <div>
          ...
          {emergency.length > 0 && (
            <div style={{ margin:'0 1.2rem .8rem', background:'#FFF8E1', border:'1.5px solid #FFE082', borderRadius:'14px', padding:'.9rem 1.1rem' }}>
              <div style={{ fontSize:'.78rem', fontWeight:800, color:'#F59E0B', textTransform:'uppercase', letterSpacing:'.06em', marginBottom:'.5rem' }}>Info importante</div>
              {emergency.map(function(e, i) { return <div key={i} style={{ fontSize:'.82rem', color:'#1A1A2E', fontWeight:600, marginBottom:'.25rem' }}>{e}</div> })}
            </div>
          )}
          ...
          <div style={{ padding:'0 1.2rem .8rem', display:'flex', flexDirection:'column', gap:'.7rem' }}>
            ...
            <div>
              <span style={{ fontSize:'.78rem', fontWeight:800, color:'#6B7280', letterSpacing:'.06em', textTransform:'uppercase', marginBottom:'.4rem', display:'block' }}>Tu número (opcional)</span>
              <input
                type="tel"
                value={finderPhone}
                onChange={function(e) { setFinderPhone(e.target.value) }}
                placeholder="Ej: +58 412 700 0977"
                style={{ width:'100%', background:'#F9F7F4', border:'1.5px solid #E5E7EB', borderRadius:'12px', padding:'.75rem 1rem', fontFamily:'Nunito,sans-serif', fontSize:'.88rem', fontWeight:600, color:'#1A1A2E', outline:'none', boxSizing:'border-box' }}
              />
              <span style={{ fontSize:'.72rem', color:'#9CA3AF', fontWeight:600, marginTop:'.3rem', display:'block' }}>Así el dueño puede contactarte directamente</span>
            </div>
            ...
            <button
              ...
              style={{ background:'#25D366', color:'white', borderRadius:'14px', padding:'.85rem', fontSize:'.88rem', fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center', gap:'.6rem', border:'none', cursor:'pointer' }}
            >
          ...
          <div style={{ padding:'0 1.2rem 1.2rem' }}>
            <span style={{ fontSize:'.78rem', fontWeight:800, color:'#6B7280', letterSpacing:'.06em', textTransform:'uppercase', marginBottom:'.6rem', display:'block' }}>Escribele algo al dueño</span>
            <textarea
              value={msgValue}
              onChange={function(e) { setMsgValue(e.target.value) }}
              disabled={msgSent}
        ...
        {history.length > 0 && (
          <div style={{ width:'100%', maxWidth:'420px', background:'white', borderRadius:'24px', border:'2px solid #F3F4F6', boxShadow:'0 8px 30px rgba(0,0,0,.06)', overflow:'hidden', position:'relative', zIndex:1 }}>
            ...
            {history.slice(0,5).map(function(s, i) {
