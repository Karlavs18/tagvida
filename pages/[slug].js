    ...
    try {
      await fetch('/api/scan', {
        ...
        body:    JSON.stringify({
          ...
          finder_phone: finderPhone || null,  // ← NUEVO
        }),
      })
    } catch(e) {
...
onClick={() => {
    const phone = (dije.owner_whatsapp||'').replace(/\D/g,'')
    const msg = `*Tag Vida - Encontre a ${dije.name}!*\n\n${userLoc ? `https://maps.google.com/?q=${userLoc.lat},${userLoc.lng}` : 'sin ubicacion'}\n\nDireccion: ${locLabel.replace('📍 ','')}\nHora: ${new Date().toLocaleString('es-MX')}${finderPhone ? `\nMi numero: ${finderPhone}` : ''}`
    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(msg)}`, '_blank')
  }}
  style={{ background:'#25D366', color:'white', borderRadius:'14px', padding:'.85rem', fontSize:'.88rem', fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center', gap:'.6rem', textDecoration:'none', cursor:'pointer' }}
>
